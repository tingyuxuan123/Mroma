 ...
 * - 监听 mroma:clear-context 和 mroma:focus-input 自定义事件
 * - 卡片式容器样式
 */
...
export function ChatInput({ conversationId, streaming, pendingAttachments, onSetPendingAttachments, onSend, onStop, onClearContext }: ChatInputProps): React.ReactElement {
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('mroma:clear-context', handler)
    return () => window.removeEventListener('mroma:clear-context', handler)
  }, [onClearContext])
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('mroma:focus-input', handler)
    return () => window.removeEventListener('mroma:focus-input', handler)
  }, [])
