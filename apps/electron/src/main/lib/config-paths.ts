 ...
 * 获取配置目录名称
 *
 * 开发模式下返回 '.mroma-dev'，正式版本返回 '.mroma'。
 *
 * 检测优先级：
 * 1. MROMA_DEV=1 环境变量（显式覆盖）
...
export function getConfigDirName(): string {
  if (_configDirName === undefined) {
    if (process.env.MROMA_DEV === '1') {
      _configDirName = '.mroma-dev'
    } else {
      try {
        const { app } = require('electron')
        _configDirName = app.isPackaged ? '.mroma' : '.mroma-dev'
      } catch {
        _configDirName = '.mroma'
      }
    }
    const mode = _configDirName === '.mroma-dev' ? '开发模式' : '正式版本'
 ...
 * 获取对话索引文件路径
 *
 * @returns ~/.mroma/conversations.json
 */
 ...
 *
 * @param id 对话 ID
 * @returns ~/.mroma/conversations/{id}.jsonl
 */
 ...
 * @returns ~/.mroma/attachments/{conversationId}/
 */
 ...
 * @returns 完整路径 ~/.mroma/attachments/{conversationId}/{uuid}.ext
 */
 ...
 * 获取用户档案文件路径
 *
 * @returns ~/.mroma/user-profile.json
 */
 ...
 * 获取代理配置文件路径
 *
 * @returns ~/.mroma/proxy-settings.json
 */
 ...
 * 获取系统提示词配置文件路径
 *
 * @returns ~/.mroma/system-prompts.json
 */
 ...
 * 获取 Agent 会话索引文件路径
 *
 * @returns ~/.mroma/agent-sessions.json
 */
 ...
 *
 * @param id 会话 ID
 * @returns ~/.mroma/agent-sessions/{id}.jsonl
 */
 ...
 * 获取 Agent 工作区索引文件路径
 *
 * @returns ~/.mroma/agent-workspaces.json
 */
 ...
 * 如果目录不存在则自动创建。
 *
 * @returns ~/.mroma/agent-workspaces/
 */
 ...
 *
 * @param slug 工作区 slug
 * @returns ~/.mroma/agent-workspaces/{slug}/
 */
 ...
 *
 * @param slug 工作区 slug
 * @returns ~/.mroma/agent-workspaces/{slug}/mcp.json
 */
 ...
 *
 * @param slug 工作区 slug
 * @returns ~/.mroma/agent-workspaces/{slug}/skills/
 */
 ...
 *
 * @param slug 工作区 slug
 * @returns ~/.mroma/agent-workspaces/{slug}/workspace-files/
 */
 ...
 *
 * @param slug 工作区 slug
 * @returns ~/.mroma/agent-workspaces/{slug}/skills-inactive/
 */
...
/**
 * 从 app bundle 同步默认 Skills 到 ~/.mroma/default-skills/
 *
 ...
 * 获取微信长轮询同步游标路径
 *
 * @returns ~/.mroma/wechat-sync.json
 */
 ...
 * 获取飞书聊天绑定持久化路径
 *
 * @returns ~/.mroma/feishu-bindings.json
 */
 ...
 *
 * @returns ~/.mroma/feishu-bindings-{botId}.json
 */
 ...
 * @returns ~/.mroma/agent-workspaces/{slug}/{sessionId}/
 */
 ...
 * 而不是用户的 ~/.claude.json，实现 Mroma 与 Claude Code CLI 的配置隔离。
 *
 * 如果目录不存在则自动创建。
 *
 * @returns ~/.mroma/sdk-config/
 */
