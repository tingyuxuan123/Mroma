...
export const agentSidePanelOpenAtom = atomWithStorage<boolean>('mroma-agent-sidepanel-open', true)

/** 侧面板宽度（全局共享，用户拖拽后持久化） */
export const agentSidePanelWidthAtom = atomWithStorage<number>('mroma-agent-sidepanel-width', 280)
...
/** 新会话默认权限模式 */
export const agentDefaultPermissionModeAtom = atom<MromaPermissionMode>(MROMA_DEFAULT_PERMISSION_MODE)

/** Per-session 权限模式 Map — sessionId → MromaPermissionMode */
export const agentPermissionModeMapAtom = atom<Map<string, MromaPermissionMode>>(new Map())

/**
 * 按 sessionId 派生该 session 的持久化权限模式。
 * 返回 `undefined`（session 不存在或未设置）或具体的 MromaPermissionMode 字符串，
...
export function applyAgentEvent(
  ...
  switch (event.type) {
    ...
    case 'complete':
      ...
      // result.usage 是整次 Agent 任务的累计用量；ContextUsageBadge 需要展示最近一次模型调用
      // 的窗口占用。已有 usage_update 时不覆盖，仅在第三方渠道缺少流式 usage 时兜底填充。
      const shouldUseResultUsage = prev.inputTokens == null
      return {
        ...prev,
        retrying: undefined,
